...
proc activating_link {name} {
  if {[string length $name] > 0} {
    ...
    spellWord $name;
    puts "$name: link activated";
    if {[string tolower $name] eq "rolink"} {
       playSilence 350;
       playMsg "Core" "rolink";
       playSilence 350;
    }
...
proc link_already_active {name} {
  if {[string length $name] > 0} {
    ...
    spellWord $name;
    puts "$name: link not already active";
  }
}
